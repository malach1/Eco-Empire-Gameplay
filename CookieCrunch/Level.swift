...
class Level {
  ...
  func removeMatches() -> Set<Chain> {
    ...
    // MARK: *REMOVE ROWCOLUMN
    func removeRowColumn() -> Set<Chain> {
        
        if directionOfSwipe == "left" || directionOfSwipe == "right" {
            let horizontalChains = DetectHorizontalRemoval(rowColumn)
            removeCookies(horizontalChains)
            return horizontalChains
        } else {
            let verticalChains = DetectVerticalRemoval(rowColumn)
            removeCookies(verticalChains)
...
/*
    func removeRowColumn() -> Set<Chain> {
        ...
        print("horizontal: \(horizontalChains.count)")
        print("vertical: \(verticalChains.count)")
        print("horizontal: \(horizontalChains)")
        
        if (directionOfSwipe == "left" || directionOfSwipe == "right") {
            removeCookies(horizontalChains)
            return horizontalChains
        } else if (directionOfSwipe == "up" || directionOfSwipe == "down") {
            removeCookies(verticalChains)
            return verticalChains
        } else {
            return emptyChains
...
*/
    ...
    // MARK: *HoritzontalRemoval
    func DetectHorizontalRemoval(row: Int) -> Set<Chain> {
        ...
        for var column = 0; column < NumColumns - 2 ; {
            ...
            if let cookie = cookies[column, row] {
                ...
                repeat {
                if let cookieThatExists = cookies[column, row] {
                        chain.addCookie(cookieThatExists)
                        ++column
                } else {
                    ++column
                    }
    ...
    // MARK: *VeritcalRemoval
    func DetectVerticalRemoval(column: Int) -> Set<Chain> {
        // Contains the Cookie objects that were part of a horizontal chain.
        // These cookies must be removed.
        var set = Set<Chain>()
        ...
        for var row = 0; row < NumRows - 2 ; {
            ...
            if let cookie = cookies[column, row] {
                ...
                let chain = Chain(chainType: .Horizontal)
                repeat {
                if let existingCookies = cookies[column, row] {
                        chain.addCookie(existingCookies)
                        ++row
                } else {
                    ++row
                    }
  ...
  func topUpCookies() -> [[Cookie]] {
    ...
    for column in 0..<NumColumns {
      var array = [Cookie]()
        ...
        if directionOfSwipe == "left" || directionOfSwipe == "right" {
            //Fill holes at row
            for var rowToAdd = rowColumn; rowToAdd >= 0 && cookies[column, rowToAdd] == nil; --rowToAdd {
                // Found a hole?
                if tiles[column, rowToAdd] != nil {
                    ...
                    let cookie = Cookie(column: column, row: rowToAdd, cookieType: cookieType)
                    cookies[column, rowToAdd] = cookie
                    array.append(cookie)
                }
            }
        }
