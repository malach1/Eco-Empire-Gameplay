...
class GameScene: SKScene {
  ...
  func pointForColumn(column: Int, row: Int) -> CGPoint {
    ...
    //starts at row: 6
    func pointForReverseColumn(column: Int, row: Int) -> CGPoint {
        return CGPoint(
            x: CGFloat(column)*TileWidth + TileWidth/2,
            y: -(CGFloat(row)*TileHeight + TileHeight/2))
    }
    
    func pointForColumnDouble(column: Int, row: Double) -> CGPoint {
        return CGPoint(
            x: CGFloat(column)*TileWidth + TileWidth/2,
            y: CGFloat(row)*TileHeight + TileHeight/2)
    }
  ...
  override func touchesMoved(touches: Set<UITouch>, withEvent event: UIEvent?) {
    ...
    if success {
      ...
      if column < swipeFromColumn! {          // swipe left
        ...
        rowColumn = swipeFromColumn!
      } else if row > swipeFromRow! {         // swipe up
        vertDelta = 1
        directionOfSwipe = "up"
        rowColumn = swipeFromColumn!
      }
        print("direction \(directionOfSwipe)")
  ...
  func animateNewCookies(columns: [[Cookie]], completion: () -> ()) {
    ...
    for array in columns {
      ...
      // in the array, which is always the top-most one for this column.
       ...
       for (idx, cookie) in array.enumerate() {
        ...
        let duration = NSTimeInterval(startRow - cookie.row) * 0.2
    ...
    // MARK: **Replace cookies
    func animateReplaceCookies(columns: [[Cookie]], completion: () -> ()) {
        // We don't want to continue with the game until all the animations are
        // complete, so we calculate how long the longest animation lasts, and
        // wait that amount before we trigger the completion block.
        var longestDuration: NSTimeInterval = 0
        ...
        for array in columns {
            ...
            // MARK: *Changes* startRow is where the cookies will animate from.
            let startRow = array[0].row + 1
            let startRowDouble:Double = Double(array[0].row) + 0.00001
            for (idx, cookie) in array.enumerate() {
                
                // Create a new sprite for the cookie.
                let sprite = SKSpriteNode(imageNamed: cookie.cookieType.spriteName)
                
                // MARK: *Changes* Depending on the swipe indicates which direction the cookies will arrive from.
                
               
                if directionOfSwipe == "right" {
                    sprite.position = pointForColumnDouble(0, row: startRowDouble)
                } else if directionOfSwipe == "left" {
                    sprite.position = pointForColumnDouble(11, row: startRowDouble)
                } else if directionOfSwipe == "up"{
                    sprite.position = pointForReverseColumn(cookie.column, row: 6)
                } else if directionOfSwipe == "down" {
                    sprite.position = pointForColumn(cookie.column, row: 6)
                }
                ...
                // Calculate duration based on far the cookie has to fall.
                // MARK: *Changes* Speed of transition
                
                var duration = NSTimeInterval(startRow - cookie.row) * 0.5
                
                if directionOfSwipe == "up" || directionOfSwipe == "down"{
                     duration = NSTimeInterval(startRow - cookie.row) * 0.1
                } else {
                    duration = NSTimeInterval(startRow - cookie.row) * 0.1
                }
                ...
                // Animate the sprite falling down. Also fade it in to make the sprite
                // appear less abruptly.
                var newPosition = pointForColumn(cookie.column, row: cookie.row)
                
                if directionOfSwipe == "up" {
                   newPosition = pointForColumn(cookie.column, row: 5-cookie.row)
                }
